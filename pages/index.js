...
export default function Home({ initialPage }) {
  ...
  const { status, data, error, fetchNextPage } = useInfiniteQuery(
    ["products", query.season || "", query.gender || "", query.ageGroup || ""],
    fetchResults,
    {
  ...
  return (
    <ProductListLayout below={<><Info /><Contact /><AboutUs /></>}>
      <Wrapper>
        {status === "loading" ? (
          <p>Loading...</p>
        ) : status === "error" ? (
...
const Wrapper = styled.div`
  text-align: center;
  .ref-page-ending { display: block; height: 16rem; margin-top: -10rem; }
`;

const Gallery = styled.div`
  display: grid;
  grid-template-columns: 1fr 1fr 1fr 1fr;
  @media screen and (max-width: 900px) { grid-template-columns: 1fr 1fr 1fr; }
...
const Product = styled.div`
  ...
  @media screen and (max-width: 768px) {
    :nth-of-type(odd) { margin-right: 0.5rem; margin-left: 0; }
    :nth-of-type(even) { margin-right: 0; margin-left: 0.5rem; }
  }
  .productImage { transition: all 0.3s ease-out; }
  .productInfo { .productDetails { margin: 1rem 0; .code { font-weight: bold; } .name { max-lines: 2; } } }
`;
